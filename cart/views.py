...
def cart_items(request):
    ...
    try:
        ...
        if len(cartitems) == 0:
            ...
            #new_cartitems = returns_item(cartitems,"mini")
            #try:
            #    for item in new_cartitems.reverse():
            #        pass
            #except TypeError:
            #    pass
            ...
            if request.method == "POST":
                ...
                def return_protein(item):#here protein return was defined to return the particular item of the protein change.
                    if item.content_type == food_model:
                        return [item.content_object.food_item,cartitemfood.total_price(item.food_category)]
                    elif item.content_type == soup_model:
                        return [item.content_object.soup_item,cartitemfood.total_price(item.food_category)]
